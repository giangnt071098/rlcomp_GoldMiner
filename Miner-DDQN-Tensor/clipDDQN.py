...
class DDQN(object):
	def __init__(
		...
		epsilon_decay = 0.9992,#The decay epislon for each update_epsilon time
		discount=0.99,
		learning_rate = 3e-4,
		replace = 1000
	):
		...
		self.optimizer1 = torch.optim.RMSprop(self.model1.parameters(), lr = learning_rate)
		self.optimizer2 = torch.optim.RMSprop(self.model2.parameters(), lr = learning_rate)
