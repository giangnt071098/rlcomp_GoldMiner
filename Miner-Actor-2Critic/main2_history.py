...
import utils
import TD3, DDPG, newDDPG, A2C, newTD3, TD3_history
...
def eval_policy(policy, eval_env, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        obvers = np.concatenate([state, state, state, state])
        while not done:
            action, _ = policy.select_action(np.array(obvers))
            eval_env.step(str(action)) 
            next_state = minerEnv.get_state2(int(args.limit_obs))
            next_obvers = np.append(next_state, obvers[:INPUTNUM//4*3])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy", default="TD3_history")                 # Policy name (TD3, DDPG or OurDDPG)
    parser.add_argument("--env", default="Miner")          # OpenAI gym environment name
    parser.add_argument("--notice", default="history") 
    ...
    parser.add_argument("--epsilon", default=1)                   # Discount factor
    ...
    print("---------------------------------------")
    INPUTNUM = ((2*int(args.limit_obs) + 1)**2 +3)*4 #198 #The number of input values for the DQN model
    ...
    if args.policy == "TD3_history":
        policy = TD3_history.TD3(**kwargs)
    ...
    for episode_i in range(0, N_EPISODE):
        ...
        for step in range(0, maxStep):
            ...
            action, action_probs = policy.select_action(np.array(obvers))
            ...
            replay_buffer.add(obvers, action_probs, next_obvers, reward, done_bool)
        ...
        print(policy.predict_action(obvers)[1])
