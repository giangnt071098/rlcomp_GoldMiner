...
class Actor(nn.Module):
	...
	def forward(self, state):
		...
		return F.softmax(self.l3(a), dim =-1), self.l3(a)
...
class TD3(object):
	...
	def select_action(self, state):
		...
		distribution_action, l3 = self.actor.forward(state)
		a_probs = distribution_action.cpu().data.numpy().flatten()
	...
	def predict_action(self, state):
		...
		distribution_action, l3 = self.actor.forward(state)
		a_max = np.argmax(distribution_action.cpu().data.numpy().flatten())
		return a_max, l3[0].detach().numpy()
