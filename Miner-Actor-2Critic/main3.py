...
def eval_policy(policy, eval_env, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        eval_env.reset()
        state, done = eval_env.get_state3(int(args.limit_obs)), False
        while not done:
            action, _ = policy.select_action(np.array(state))
            eval_env.step(str(action)) 
            next_state = eval_env.get_state3(int(args.limit_obs))
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    INPUTNUM = (2*int(args.limit_obs) + 1)**2 +3 + 3#198 #The number of input values for the DQN model
    ...
    for episode_i in range(0, N_EPISODE):
        ...
        state = minerEnv.get_state3(int(args.limit_obs))
        print(state)
        done = False
        ...
        for step in range(0, maxStep):
            ...
            next_state = minerEnv.get_state3(int(args.limit_obs))
