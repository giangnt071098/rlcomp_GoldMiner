...
class MinerEnv:
    ...
    def get_reward(self):
        ...
        if self.state.mapInfo.gold_amount(self.state_x_pre, self.state_y_pre) >= 50 and self.state.lastAction != 5: # in gold but don't craft
            self.in_gold += 1
            reward -= 0.4*self.in_gold
        if self.state.lastAction == 5 and score_action == 0: # not in gold but craft
            self.craft_no_gold +=1
            reward -= self.craft_no_gold*0.5
        # if len(self.last3position) == 3 and self.state.lastAction !=5: # back to same position
        #     if self.last3position[0] == self.last3position[2]:
        #         reward -= 0.1
        #     if self.last3position[1] == self.last3position[2]:
    ...
    def get_state_tensor(self, scale_map):
        n = scale_map
        view = np.zeros((n*(self.state.mapInfo.max_x + 1), n*(self.state.mapInfo.max_y + 1), 6))
        for i in range(self.state.mapInfo.max_x + 1):
            for j in range(self.state.mapInfo.max_y + 1):
            ...
            if player["playerId"] != self.state.id:
                try:
                    if player["status"] not in [1,2,3]:
                        try:
                            view[n*player["posx"]:n*player["posx"]+n,n*player["posy"]:n*player["posy"]+n,stt + 1] = player["energy"]/50
                        except:
                            view[n*player["posx"]:n*player["posx"]+n,n*player["posy"]:n*player["posy"]+n,stt + 1] = 1
                except:
                    view[n*player["posx"]: n*player["posx"]+n,n*player["posy"]:n*player["posy"]+n,stt+1]= 1
    ...
    def get_state_tensor2(self, scale_map):
        ...
        for i in range(self.state.mapInfo.max_x + 1):
            for j in range(self.state.mapInfo.max_y + 1):
                if self.state.mapInfo.get_obstacle(i, j) == TreeID:  # Tree     # trap map
                    view[n*i:n*i+n, n*j:n*j+n,0] = -TreeID
                    view[n*i:n*i+n, n*j:n*j+n,1] -= (randrange(16)+5)/50
                if self.state.mapInfo.get_obstacle(i, j) == TrapID:  # Trap     # trap map
                    view[n*i:n*i+n, n*j:n*j+n,0] = -TrapID
                    view[n*i:n*i+n, n*j:n*j+n,1] += -10/50
                if self.state.mapInfo.get_obstacle(i, j) == SwampID: # Swamp    # trap map
                    view[n*i:n*i+n, n*j:n*j+n,0] = -SwampID
                    view[n*i:n*i+n, n*j:n*j+n,1] += -100/50
                if self.state.mapInfo.gold_amount(i, j) > 0:
                    view[n*i:n*i+n, n*j:n*j+n,0] = self.state.mapInfo.gold_amount(i, j)/1000 ##/10 gold map
                    view[n*i:n*i+n, n*j:n*j+n,5] = self.state.mapInfo.gold_amount(i, j)/1000
                    view[n*i:n*i+n, n*j:n*j+n,1] += -4/50
                if self.state.mapInfo.get_obstacle(i, j) == LandID:
                    view[n*i:n*i+n, n*j:n*j+n,1] += -1/50
        index = 1
        for stt,player in enumerate(self.state.players):
            if player["playerId"] != self.state.id:
                try:
                    if player["status"] not in [1,2,3]:
                        try:
                            view[n*player["posx"]:n*player["posx"]+n,n*player["posy"]:n*player["posy"]+n,index + 1] += player["energy"]/50
                        except:
                            view[n*player["posx"]:n*player["posx"]+n,n*player["posy"]:n*player["posy"]+n,index + 1] += 1
                except:
                    view[n*player["posx"]: n*player["posx"]+n,n*player["posy"]:n*player["posy"]+n,index+1] += 1
                    # print(self.state.players)
                    #print(view[player["posx"]: player["posx"]+1, player["posy"]: player["posy"]+1, stt])
