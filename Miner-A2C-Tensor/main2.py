...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    INPUTNUM = (2*int(args.limit_obs) + 1)**2 +3#198 #The number of input values for the DQN model
    ...
    for episode_i in range(0, N_EPISODE):
        ...
        for step in range(0, maxStep):
            ...
            action_bot, bot_probs = policy_bot.predict_action(state_bot)
            ##

            action, action_probs = policy.select_action(np.array(state), bot_probs, action_bot)
            set_action.append(action)
            minerEnv.step(str(action))
            next_state = minerEnv.get_state2(int(args.limit_obs))
            reward = minerEnv.get_reward2()
