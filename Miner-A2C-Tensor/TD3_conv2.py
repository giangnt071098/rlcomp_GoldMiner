...
class Actor(nn.Module):
	def __init__(self, state_dim, action_dim, max_action, dropout= 0.4):
		...
		self.conv1 = nn.Conv2d(state_dim[0], 16, 3, stride = 1, padding = 1)
		...
		self.l3 = nn.Linear(fc_input_dims, action_dim)
	...
	def forward(self, state):
		...
		conv = F.relu(self.maxp3(self.conv3(conv)))
		conv = F.relu(self.maxp4(self.conv4(conv)))
		flatten = conv.view(conv.size()[0], -1)
		 
		a = F.relu(self.l3(flatten))
...
class Critic(nn.Module):
	def __init__(self, state_dim, action_dim, dropout = 0.4):
		...
		# Q1 architecture
		self.conv1_1 = nn.Conv2d(state_dim[0], 16, 3, stride = 1, padding = 1)
		self.maxp1_1 = nn.MaxPool2d(2, 2)
		self.conv3_1 = nn.Conv2d(16, 16, 3, stride=1, padding=1)
		self.maxp3_1 = nn.MaxPool2d(2, 2)
		self.conv4_1 = nn.Conv2d(16, 16, 3, stride=1, padding=1)
		self.maxp4_1 = nn.MaxPool2d(2, 2)

		fc_input_dims = self.calculate_conv_output_dims(state_dim)
		self.l1 = nn.Linear(fc_input_dims + action_dim, 1)
		#self.l3 = nn.Linear(256, 1)
		
		# Q2 architecture
		self.conv1_2 = nn.Conv2d(state_dim[0], 16, 3, stride = 1, padding = 1)
		self.maxp1_2 = nn.MaxPool2d(2, 2)
		self.conv3_2 = nn.Conv2d(16, 16, 3, stride=1, padding=1)
		self.maxp3_2 = nn.MaxPool2d(2, 2)
		self.conv4_2 = nn.Conv2d(16, 16, 3, stride=1, padding=1)
		self.maxp4_2 = nn.MaxPool2d(2, 2)
		#fc_input_dims = self.calculate_conv_output_dims(state_dim)

		self.l4 = nn.Linear(fc_input_dims + action_dim, 1)
	...
	def forward(self, state, action):
		conv1 = F.relu(self.maxp1_1(self.conv1_1(state)))
		conv1 = F.relu(self.maxp3_1(self.conv3_1(conv1)))
		conv1 = F.relu(self.maxp4_1(self.conv4_1(conv1)))
		...
		# Q2
		conv2 = F.relu(self.maxp1_2(self.conv1_2(state)))
		conv2 = F.relu(self.maxp3_2(self.conv3_2(conv2)))
		conv2 = F.relu(self.maxp4_2(self.conv4_2(conv2)))
		flatten2 = conv2.view(conv2.size()[0], -1)
		sb = torch.cat([flatten2, action], 1)

		#q2 = F.relu(self.l4(sb))
		q2 = self.l4(sb)
		return q1, q2
	...
	def calculate_conv_output_dims(self, input_dims):
		...
		dims = self.maxp1_1(dims)
		dims = self.conv3_1(dims)
		dims = self.maxp3_1(dims)
