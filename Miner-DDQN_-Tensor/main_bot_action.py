...
import utils
import DDQN, clipDDQN, DuelingDQN
...
def eval_policy(policy, eval_env, eval_episodes=10):
    ...
    for _ in range(eval_episodes):
        ...
        eval_env.reset()
        state, done = eval_env.get_state_tensor2(), False
        while not done:
            action = policy.select_action(np.array(state))
            eval_env.step(str(action)) 
            next_state = eval_env.get_state_tensor2()
...
if __name__ == "__main__":
    ...
    print("---------------------------------------")
    scale_map = 1
    INPUTNUM = (8, 21*scale_map, 9*scale_map)#198 #The number of input values for the DQN model
    ...
    for episode_i in range(0, N_EPISODE):
        ...
        total_reward = 0
        observ = np.reshape([np.stack((state, state, state, state), axis = 0)], (8, INPUTNUM[1], INPUTNUM[2]))
        ...
        for step in range(0, maxStep):
            ...
            action = policy.select_action(observ, action_bot)
            set_action.append(action)
            minerEnv.step(str(action))
            next_state = minerEnv.get_state_tensor2(scale_map)
            next_observ = np.append(np.reshape(next_state, (2, INPUTNUM[1], INPUTNUM[2])), observ[:6, :, :], axis = 0)
            reward = minerEnv.get_reward()
            done = minerEnv.check_terminate()
            done_bool = float(done)
            replay_buffer.add(observ, action, next_observ, reward, done_bool)
